-- CI-only compatibility shim for historical analytics views.
-- A tracked cleanup drops match_player_intervals with CASCADE, which removes
-- these dependent views. A later tracked migration expects them to exist.
-- Recreate their historical contracts only in the disposable local chain.
...
goal_stats as (
  select
    ...
    count(*) filter (
      where g.team::text = 'as_grinta' and g.scorer_profile_id is not null
    )::integer as goals
  from public.goals g
...
assist_stats as (
  select
    ...
    count(*) filter (
      where g.team::text = 'as_grinta' and g.assist_profile_id is not null
    )::integer as assists
  from public.goals g
  join public.matches m on m.id = g.match_id
  where m.status in ('termine', 'archive')
