...
class _AdminSquadPlanPageState extends ConsumerState<AdminSquadPlanPage> {
  ...
  Future<void> _loadWorkspace(String matchId) async {
    ...
    try {
      ...
      final compositionRepository =
          ref.read(matchCompositionRepositoryProvider);
  ...
  int _playerOrder(ConvocationPlayer a, ConvocationPlayer b) {
    final byPosition = (a.waitlistPosition ?? 1 << 20)
        .compareTo(b.waitlistPosition ?? 1 << 20);
    return byPosition != 0
        ? byPosition
  ...
  Future<void> _saveEffectif() async {
    ...
    try {
      ...
      await repository.configureMatch(
      ...
      for (final player in convocations.players) {
        if (player.isGuest ||
            !player.isAvailable ||
            player.seasonPlayerId.isEmpty) {
          continue;
        }
        final wanted = _desiredConvoked.contains(player.participantId)
            ? ConvocationStatus.convoked
            : ConvocationStatus.notConvoked;
        if (player.convocationStatus == wanted && player.manualOverride)
          continue;
        await repository.setConvocation(
          matchId: convocations.matchId,
          seasonPlayerId: player.seasonPlayerId,
          status: wanted,
          turnShouldConsume: wanted == ConvocationStatus.notConvoked,
          reason: 'Effectif enregistré depuis le match',
        );
      }
      await repository.publishMatch(
        matchId: convocations.matchId,
        reason: 'Effectif enregistré depuis le match',
      );
  ...
  void _applyFormation(String code) {
    ...
    for (final entry in field) {
      ...
      free.sort(
        (a, b) => (current - a.position)
            .distance
            .compareTo((current - b.position).distance),
      );
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    for (final defenders in const [3, 4, 5]) ...[
                      ...
                      for (final item
                          in footballFormationsForDefenders(defenders))
                        DropdownMenuItem(
  ...
  Widget build(BuildContext context) {
    final chip = ActionChip(
      avatar: player.isGuest
          ...
          : null,
      label: Text(player.firstName.trim().isEmpty
          ? player.displayName
