...
class StatisticsRepository {
  ...
  Future<List<PlayerStatistics>> fetchCareerStatistics() async {
    final profilesResponse = await _client.from('profiles').select().eq('status', 'active');
    final statsResponse = await _client.from('v_player_career_stats').select('''
      profile_id,matches_played,goals,assists,penalty_faults,motm,clean_sheets,
      minutes_played,starts,substitute_appearances
    ''');

    final statsByProfile = <String, Map<String, dynamic>>{};
    for (final raw in statsResponse as List) {
      final row = Map<String, dynamic>.from(raw);
      statsByProfile[row['profile_id'].toString()] = row;
    }

    return (profilesResponse as List).map((raw) {
      ...
      return PlayerStatistics(
        ...
        penaltyFaults: int.tryParse('${stats['penalty_faults'] ?? 0}') ?? 0,
        ...
        substituteAppearances: int.tryParse('${stats['substitute_appearances'] ?? 0}') ?? 0,
      );
    }).toList();
  }
}
...
final careerStatisticsProvider = FutureProvider<List<PlayerStatistics>>(
  (ref) => ref.watch(statisticsRepositoryProvider).fetchCareerStatistics(),
);
