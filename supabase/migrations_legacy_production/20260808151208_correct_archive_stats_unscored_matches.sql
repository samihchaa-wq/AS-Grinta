...
begin
  if exists (
    select 1 from public.historical_team_statistics
    where scope = 'all_time' and matches_played = 313
  ) then
    ...
    where h.scope = d.scope and h.player_name = d.player_name;
...
as
with baseline as (
  select through_season, recent_results, score_margin_distribution
  from public.historical_team_statistics
  where scope = 'all_time'
), post_baseline as (
  select m.id, m.match_date,
         case when m.score_as_grinta > m.score_adverse then 'V'
  ...
  select *, row_number() over (order by match_date desc, id desc) as rn
  from post_baseline
), post_recent as (
  select coalesce(array_agg(result order by match_date, id) filter (where rn <= 6), array[]::text[]) as results,
  ...
  from baseline b cross join post_recent p
), margin_parts as (
  select key::int as margin, value::int as cnt
  from baseline b, lateral jsonb_each_text(coalesce(b.score_margin_distribution,'{}'::jsonb))
  union all
  select margin, count(*)::int
  from post_baseline
  group by margin
), all_time_margins as (
  select coalesce(jsonb_object_agg(margin::text, cnt order by margin), '{}'::jsonb) as distribution
  from (
    select margin, sum(cnt)::int as cnt
    from margin_parts
    group by margin
  ) x
)
select source.period_key,
       ...
       case
         ...
         when source.period_key = 'all_time' then (select results from recent_all_time)
         else source.recent_results
       end as recent_results,
       case
         when source.period_key = 'all_time' then (select distribution from all_time_margins)
