...
alter table public.profiles
  ...
  check (role = any (array['pronostiqueur'::text, 'admin'::text, 'moderateur'::text, 'coach'::text]));
...
create table if not exists public.players (
  ...
  constraint players_names_not_blank check (btrim(first_name) <> '' and btrim(last_name) <> '')
);

create index if not exists players_active_idx on public.players (is_active, first_name, last_name);
create index if not exists players_claim_token_idx on public.players (claim_token) where claim_token is not null;
