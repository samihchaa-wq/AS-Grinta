...
using (
 ...
 or (
  public.is_admin()
  and (controller_profile_id=auth.uid() or (controller_profile_id is null and controller_session_id is null))
 )
)
with check (
 public.is_moderator()
...
begin
 if new.status is distinct from old.status then
  if old.status='running' and old.clock_started_at is not null then
   new.elapsed_seconds:=old.elapsed_seconds+greatest(0,floor(extract(epoch from(now()-old.clock_started_at)))::int);
  else
...
create trigger trg_guard_live_session_update before update on public.live_sessions for each row execute function public.guard_live_session_update();

create or replace function public.apply_substitution_to_positions()
returns trigger language plpgsql security definer set search_path=public as $$
declare out_player uuid; out_slot text;
begin
 if new.action<>'in' then return new; end if;
 select s.profile_id into out_player
 from public.substitutions s
 where s.live_session_id=new.live_session_id and s.minute=new.minute and s.action='out' and s.created_at<=new.created_at
 order by s.created_at desc,s.id desc limit 1;
 if out_player is null then return new; end if;
 select slot_code into out_slot from public.live_positions where live_session_id=new.live_session_id and profile_id=out_player;
 if out_slot is null then return new; end if;
 delete from public.live_positions where live_session_id=new.live_session_id and profile_id in(out_player,new.profile_id);
 insert into public.live_positions(live_session_id,profile_id,slot_code) values(new.live_session_id,new.profile_id,out_slot);
 return new;
end $$;
drop trigger if exists trg_apply_substitution_to_positions on public.substitutions;
create trigger trg_apply_substitution_to_positions after insert on public.substitutions for each row execute function public.apply_substitution_to_positions();
...
begin
 if not public.is_admin() then raise exception 'Admin role required'; end if;
 update public.matches set status='archive',updated_at=now() where id=p_match_id and status<>'archive';
 return found;
end $$;
