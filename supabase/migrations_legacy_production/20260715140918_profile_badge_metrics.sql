...
-- de l'armoire à badges (« en cours »). Lecture seule, accessible à tous les
-- membres connectés (les stats sont publiques dans le club).
...
begin
  ...
  select
    coalesce((select sum(goals) from mps), 0)::integer,
    coalesce((select count(*) from apps), 0)::integer,
    coalesce((select count(*) from mps where clean_sheet), 0)::integer,
    coalesce((select count(*) from mvp), 0)::integer,
    coalesce((select count(*) filter (where exact = 1) from public.v_match_prediction_flags where profile_id = p_profile_id), 0)::integer,
    coalesce((select count(*) filter (where bon_pari = 1) from public.v_match_prediction_flags where profile_id = p_profile_id), 0)::integer;
end;
$function$;
