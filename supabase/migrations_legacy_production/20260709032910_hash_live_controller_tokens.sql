...
as $$
  select exists(
    ...
    where ls.match_id=p_match_id
      ...
      and ls.controller_session_id=public.live_session_token_hash(p_controller_session_id)
...
begin
  ...
  where match_id=p_match_id
    ...
    and controller_session_id=public.live_session_token_hash(p_controller_session_id);
  get diagnostics affected=row_count;
  return affected=1;
end;
$$;
...
begin
  update public.live_sessions
  set controller_disconnected_at=now(),updated_at=now()
  where match_id=p_match_id
    and controller_profile_id=auth.uid()
    and controller_session_id=public.live_session_token_hash(p_controller_session_id);
  get diagnostics affected=row_count;
...
set search_path=public,extensions
as $$
declare affected integer; live_id uuid;
begin
  ...
  if p_status='running' and not exists(
    ...
    insert into public.match_player_intervals(match_id,profile_id,entered_minute,started)
