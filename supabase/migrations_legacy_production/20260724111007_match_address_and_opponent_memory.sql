...
-- Lecture au niveau colonne (aligne avec le reste : id/name déjà lisibles).
grant select (address) on public.matches to authenticated;
grant select (address) on public.opponents to authenticated;

-- Enregistre l'adresse d'un match et la mémorise sur l'adversaire.
...
begin
  ...
  -- Mémorise l'adresse sur l'adversaire (repli pour les prochains matchs).
