...
drop policy if exists live_control_handoffs_involved_read on public.live_control_handoffs;
...
begin
  ...
  if not exists(
    ...
    where id=p_target_profile_id and role='admin' and status='active'
  ) then
...
begin
  ...
  set controller_profile_id=auth.uid(),
      controller_session_id=public.live_session_token_hash(p_controller_session_id),
...
begin
  ...
  where match_id=p_match_id and from_profile_id=auth.uid();
  return true;
end;
$$;

revoke execute on function public.offer_live_control(uuid,text,uuid) from public;
revoke execute on function public.offer_live_control(uuid,text,uuid) from anon;
grant execute on function public.offer_live_control(uuid,text,uuid) to authenticated;
revoke execute on function public.accept_live_control(uuid,text) from public;
revoke execute on function public.accept_live_control(uuid,text) from anon;
grant execute on function public.accept_live_control(uuid,text) to authenticated;
revoke execute on function public.cancel_live_control_offer(uuid,text) from public;
revoke execute on function public.cancel_live_control_offer(uuid,text) from anon;
grant execute on function public.cancel_live_control_offer(uuid,text) to authenticated;;
