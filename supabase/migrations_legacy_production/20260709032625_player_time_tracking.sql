...
create index if not exists idx_match_player_intervals_match on public.match_player_intervals(match_id);
create index if not exists idx_match_player_intervals_profile on public.match_player_intervals(profile_id);
alter table public.match_player_intervals enable row level security;

drop policy if exists match_player_intervals_read_authenticated on public.match_player_intervals;
create policy match_player_intervals_read_authenticated
on public.match_player_intervals for select to authenticated using(true);
...
begin
  ...
  select id into live_id from public.live_sessions where match_id=p_match_id;
  ...
  if p_status='running' and not exists(
    ...
    where lp.live_session_id=live_id and lp.slot_code is not null
...
begin
  ...
  if exists(
    ...
    where live_session_id=sid and profile_id=p_in_profile_id and slot_code is not null
  ) then
    raise exception 'Incoming player is not on bench';
  end if;

  insert into public.substitutions(live_session_id,profile_id,action,minute)
  values(sid,p_out_profile_id,'out',p_minute),(sid,p_in_profile_id,'in',p_minute);
  ...
  where live_session_id=sid and profile_id in(p_in_profile_id,p_out_profile_id);
...
begin
  ...
  if p_score_as_grinta<0 or p_score_as_grinta>99 or p_score_adverse<0 or p_score_adverse>99 then
    raise exception 'Invalid score';
  end if;
  if not exists(
    select 1 from public.match_participants
    where match_id=p_match_id and profile_id=p_motm_profile_id
  ) then raise exception 'MOTM must be a match participant'; end if;
  if p_score_as_grinta<>(select count(*) from public.goals where match_id=p_match_id and team='as_grinta')
     or p_score_adverse<>(select count(*) from public.goals where match_id=p_match_id and team='adverse') then
    raise exception 'Score does not match recorded goals';
  end if;

  select greatest(
    planned_duration_minutes,
    coalesce((select max(minute) from public.goals where match_id=p_match_id),0),
    coalesce((select max(s.minute) from public.substitutions s join public.live_sessions ls on ls.id=s.live_session_id where ls.match_id=p_match_id),0)
  ) into final_minute
  from public.matches where id=p_match_id;

  update public.match_player_intervals
  set exited_minute=least(100,final_minute)
  where match_id=p_match_id and exited_minute is null;
  ...
  update public.live_sessions
  set status='finished',controller_profile_id=null,controller_session_id=null,
      controller_disconnected_at=null,clock_started_at=null,updated_at=now()
  where match_id=p_match_id;
...
as
with appearances as(
  select m.season_id,mp.profile_id,count(distinct mp.match_id)::integer matches_played
  ...
  select m.season_id,g.scorer_profile_id profile_id,
         count(*) filter(where g.team='as_grinta' and g.scorer_profile_id is not null)::integer goals
  from public.goals g join public.matches m on m.id=g.match_id
  where m.status in('termine','archive') group by m.season_id,g.scorer_profile_id
), assist_stats as(
  select m.season_id,g.assist_profile_id profile_id,
         count(*) filter(where g.team='as_grinta' and g.assist_profile_id is not null)::integer assists
  from public.goals g join public.matches m on m.id=g.match_id
  where m.status in('termine','archive') group by m.season_id,g.assist_profile_id
), motm_stats as(
  select m.season_id,mm.profile_id,count(*)::integer motm
  from public.match_motm mm join public.matches m on m.id=mm.match_id
  group by m.season_id,mm.profile_id
), clean_sheet_stats as(
  select m.season_id,mp.profile_id,count(*) filter(where m.score_adverse=0)::integer clean_sheets
  from public.match_participants mp
  join public.matches m on m.id=mp.match_id
  ...
  select m.season_id,i.profile_id,
         sum(greatest(0,coalesce(i.exited_minute,m.planned_duration_minutes)-i.entered_minute))::integer minutes_played,
