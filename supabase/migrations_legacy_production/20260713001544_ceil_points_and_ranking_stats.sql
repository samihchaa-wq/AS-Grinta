...
create or replace view v_season_prediction_flags as
  WITH base AS (
    SELECT sp.season_id, sp.predictor_profile_id, sp.season_player_id, sp.category,
      sp.predicted_value_30,
      CASE sp.category WHEN 'buts'::text THEN st.goals
        WHEN 'clean_sheets'::text THEN st.clean_sheets ELSE 0 END AS metric,
      s.status AS season_status, mc.matches_played
    ...
    SELECT targeted.*,
      rank() OVER (PARTITION BY season_id, season_player_id, category
        ORDER BY abs(predicted_value_30::numeric - target)) AS closeness_rank
    FROM targeted WHERE target IS NOT NULL
  )
  SELECT season_id, predictor_profile_id,
...
create or replace view v_classement_general as
  WITH mt AS (
    ...
    FROM v_match_prediction_points GROUP BY profile_id
  ), mf AS (
    SELECT profile_id, COALESCE(sum(bon_pari), 0) AS match_bons,
      COALESCE(sum(exact), 0) AS match_exacts
    FROM v_match_prediction_flags GROUP BY profile_id
  ), sp AS (
    SELECT predictor_profile_id AS profile_id,
      COALESCE(sum(points), 0::bigint)::numeric AS season_points
    FROM v_season_prediction_points GROUP BY predictor_profile_id
  ), bn AS (
    SELECT predictor_profile_id AS profile_id,
      COALESCE(sum(bonus_points), 0::bigint)::numeric AS bonus_points
    FROM v_season_prediction_bonus GROUP BY predictor_profile_id
  ), sf AS (
    SELECT predictor_profile_id AS profile_id,
      COALESCE(sum(bon_pari), 0) AS season_bons,
      COALESCE(sum(exact), 0) AS season_exacts
    FROM v_season_prediction_flags GROUP BY predictor_profile_id
  ), tot AS (
  ...
  SELECT t.profile_id, t.first_name, t.surnom, t.match_points, t.season_points,
    ceil(
    ...
    t.match_bons, t.match_exacts, t.season_bons, t.season_exacts
