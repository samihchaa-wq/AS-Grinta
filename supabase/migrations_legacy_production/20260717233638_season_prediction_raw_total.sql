create or replace view public.v_season_prediction_points as
 with eligible_seasons as (
         select seasons.id, seasons.status from seasons
         ...
         select sp.season_id, count(*) as expected_count
           from season_players sp join eligible_seasons es on es.id = sp.season_id
         ...
         select sp.season_id, sp.predictor_profile_id,
           ...
           from season_predictions sp join eligible_seasons es on es.id = sp.season_id
          group by sp.season_id, sp.predictor_profile_id
        ), eligible_predictors as (
         select pc.season_id, pc.predictor_profile_id
           from predictor_completion pc join expected_predictions ep on ep.season_id = pc.season_id
          where ep.expected_count > 0 and pc.filled_count = ep.expected_count
        ), base as (
         select sp.id, sp.season_id, sp.predictor_profile_id, sp.season_player_id, sp.category, sp.predicted_value_30,
                case sp.category when 'buts'::text then st.goals when 'clean_sheets'::text then st.clean_sheets else 0 end as metric
         ...
         select base.id, base.season_id, base.predictor_profile_id, base.season_player_id, base.category, base.predicted_value_30, base.metric,
            base.metric::numeric as target
           from base
        ), ranked as (
         select targeted.id, targeted.season_id, targeted.predictor_profile_id, targeted.season_player_id, targeted.category, targeted.predicted_value_30, targeted.target,
           ...
           from targeted where targeted.target is not null
        )
 select id, season_id, predictor_profile_id, season_player_id, category,
    ((participant_count - proximity_rank + 1) * 3 * case when predicted_value_30::numeric = target then 2 else 1 end)::integer as points
   from ranked;
...
create or replace view public.v_season_prediction_bonus as
 with eligible_seasons as (
         ...
         select sp.season_id, count(*) as expected_count
           from season_players sp join eligible_seasons es on es.id = sp.season_id group by sp.season_id
        ), predictor_completion as (
         select sp.season_id, sp.predictor_profile_id,
            count(*) filter (where sp.is_filled and (sp.category = any (array['buts'::text, 'clean_sheets'::text]))) as filled_count
           from season_predictions sp join eligible_seasons es on es.id = sp.season_id
         ...
         select pc_1.season_id, pc_1.predictor_profile_id
           from predictor_completion pc_1 join expected_predictions ep on ep.season_id = pc_1.season_id
          where ep.expected_count > 0 and pc_1.filled_count = ep.expected_count
        ), participant_count as (
         select eligible_predictors.season_id, count(*)::integer as participant_count
           from eligible_predictors group by eligible_predictors.season_id
        ), target_goals as (
         select pl.season_id, pl.id as season_player_id, st.goals::numeric as target
         ...
         select sp.season_id, sp.predictor_profile_id, sp.season_player_id, sp.predicted_value_30
           from season_predictions sp
             join eligible_predictors ep on ep.season_id = sp.season_id and ep.predictor_profile_id = sp.predictor_profile_id
             join season_players pl on pl.id = sp.season_player_id
          where sp.category = 'buts'::text and sp.is_filled and not pl.is_goalkeeper
        ), pair_scores as (
         select a.season_id, a.predictor_profile_id, count(*)::integer as total_pairs,
 ...
 select ps.season_id, ps.predictor_profile_id,
        case when ps.total_pairs = 0 or (ps.correct_pairs * 2) <= ps.total_pairs then 0
   ...
   from pair_scores ps join participant_count pc on pc.season_id = ps.season_id;;
