-- Une égalité au vote de l'Homme du match élit tous les joueurs à égalité.
-- C'est la règle du club, appliquée par private.close_match_motm_election
-- (is_winner = votes_count = max) et couverte par le test exhaustif
-- exhaustive_motm_state_space.
--
-- Le rapport d'intégrité contredisait cette règle : il comptait comme anomalie
-- tout match archivé comptant plusieurs Hommes du match. Le contrôle
-- 'historical_multiple_motm' est donc retiré ; plusieurs élus sont un résultat
-- normal, pas un défaut de données.
--
-- Aucun autre contrôle n'est modifié.
