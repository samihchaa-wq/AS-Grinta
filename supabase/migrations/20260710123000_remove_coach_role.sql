...
update public.profiles set role='moderateur' where role='coach';

alter table public.profiles drop constraint if exists profiles_role_check;
alter table public.profiles
  add constraint profiles_role_check
  check (role = any (array['pronostiqueur'::text,'admin'::text,'moderateur'::text]));
...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      and lower(coalesce(p.role::text, '')) in ('admin', 'moderateur', 'moderator')
