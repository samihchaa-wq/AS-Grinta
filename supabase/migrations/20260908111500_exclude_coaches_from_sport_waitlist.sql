...
-- Un coach n'est pas un joueur de rotation : il ne doit jamais apparaître dans
-- l'ordre global de liste d'attente, même s'il reste actif dans l'effectif de
-- saison pour ses droits Coach/Live.
delete from public.sport_waitlist_entries entry
using public.season_players player
where player.id = entry.season_player_id
  and player.season_id = entry.season_id
  and player.is_coach;
...
begin
  ...
  where entry.season_id = p_season_id
    and player.id = entry.season_player_id
    and player.season_id = entry.season_id
    and player.is_coach;
...
begin
  if new.is_coach is distinct from old.is_coach then
    ...
    if new.is_coach then
      delete from public.sport_waitlist_entries entry
      where entry.season_player_id = new.id;
    end if;
  end if;
  return new;
end;
