-- Les statistiques doivent afficher le vrai prénom des joueurs, jamais leur
-- surnom (contrairement au reste de l'app, qui privilégie le surnom depuis
-- 20260718000000_surnom_display_everywhere.sql). Le prénom réel est déjà
-- extrait côté Flutter (statistics_repository.dart) depuis le nom complet
-- renvoyé ici ; le départage des homonymes exacts (initiale du nom de
-- famille) continue donc de fonctionner sans changement.

-- Philippe Couzin est un membre du staff (présent à la quasi-totalité des
-- matchs), pas un joueur : il n'a jamais eu de fiche dans season_players et
-- ne doit pas apparaître dans les statistiques joueurs.
delete from public.historical_player_statistics
where player_name = 'Philippe Couzin';
...
current_ranked as (
  select 'current'::text as period_key, os.name as period_label,
    ...
    asp.full_name as player_name, asp.is_goalkeeper, asp.matches_played,
...
comment on view public.v_statistics_players is
  'Statistiques joueurs Actuelle/Précédente/Toutes, toujours nommés par leur vrai prénom (jamais leur surnom). team_clean_sheets compte les matchs joués avec score_adverse = 0, indépendamment du clean sheet gardien.';
