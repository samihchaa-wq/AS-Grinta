-- Les métriques CARRIÈRE des badges (présences, victoires, buts, clean sheets,
-- HDM) incluent désormais l'historique importé du club, rattaché par nom
-- complet (prénom + nom) au compte. Les métriques SAISON restent sur la
-- saison en cours uniquement. Le doublé / triplé / pronos / titres sont
-- inchangés (l'historique ne les trace pas).
...
as $function$
  with pm as (
    ...
    -- Historique importé (avant l'appli) rattaché par nom complet au compte.
    select
...
-- Recalcule les badges de tout le monde avec les nouvelles métriques carrière.
