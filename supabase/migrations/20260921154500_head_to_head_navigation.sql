...
-- Expose the source match id/type so the five head-to-head results can open
-- the corresponding calendar match sheet without guessing from date/score.
drop function if exists public.get_last_opponent_encounters(uuid);

create function public.get_last_opponent_encounters(p_match_id uuid)
returns table (
  encounter_id uuid,
  encounter_date date,
...
begin
  ...
  with all_results as (
    ...
    select distinct on (r.id)
      ...
      r.is_historical
    from all_results r
    order by r.id, r.match_date desc, r.is_historical
  )
  select
    d.id,
    d.match_date,
...
comment on function public.get_last_opponent_encounters(uuid) is
  'Returns the five latest completed matches against the opponent of a match, including source ids for calendar navigation.';

revoke execute on function public.get_last_opponent_encounters(uuid)
  from public, anon;
grant execute on function public.get_last_opponent_encounters(uuid)
