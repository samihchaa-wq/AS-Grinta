...
-- Le bilan retient les badges décrochés pendant la saison.
--
-- La date d'attribution est fiable pour cela : le recalcul des badges ignore
-- les attributions déjà en place au lieu de les réécrire, si bien qu'un badge
-- garde la date de sa première obtention.
--
-- La saison n'ayant ni date de début ni date de fin, sa fenêtre se lit sur la
-- création des saisons, prises dans l'ordre de leur nom : d'une saison à la
-- suivante. Une saison sans successeur court jusqu'au calcul.

alter table public.season_wrapped
  add column if not exists badges jsonb not null default '[]'::jsonb,
  add column if not exists badge_count integer not null default 0;

comment on column public.season_wrapped.badges is
  'Badges decroches pendant la saison, du plus ancien au plus recent : '
  '[{"code": "...", "name": "...", "emoji": "..."}, ...]. Toutes familles '
  'confondues, badges de pronostiqueur compris.';

comment on column public.season_wrapped.badge_count is
  'Nombre de badges decroches pendant la saison. Non classe : un joueur qui '
  'debute en decroche mecaniquement plus qu un ancien.';
...
begin
  ...
  -- La fenêtre de la saison : de sa création à celle de la suivante.
  season_window as (
    select
      courante.created_at as debut,
      (
        select min(suivante.created_at)
        from public.seasons suivante
        where suivante.name > courante.name
      ) as fin
    from public.seasons courante
    where courante.id = p_season_id
  ),
  earned_badges as (
    select
      ...
      jsonb_agg(
        ...
        order by award.awarded_at, badge.sort_order, badge.code
      ) as liste,
      count(*)::integer as total
    from roster
    join public.profile_badges award
      on award.profile_id = roster.profile_id
    join public.badges badge on badge.id = award.badge_id
    cross join season_window
    where award.awarded_at >= season_window.debut
      and (season_window.fin is null or award.awarded_at < season_window.fin)
    group by roster.season_player_id
  ),
  base as (
    select
      roster.season_player_id,
      ...
      coalesce(earned_badges.liste, '[]'::jsonb) as badges,
      coalesce(earned_badges.total, 0) as badge_count,
    ...
    left join earned_badges
      on earned_badges.season_player_id = roster.season_player_id
...
begin
  ...
  select jsonb_build_object(
    ...
    'badges', wrapped.badges,
    'badge_count', wrapped.badge_count
  )
  into v_result
