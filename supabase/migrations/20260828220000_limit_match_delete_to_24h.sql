...
begin
  ...
  if v_created_at is null or now() >= v_created_at + interval '24 hours' then
    raise exception 'Un match ne peut être supprimé que pendant les 24 heures suivant sa création.'
      using errcode = '22023';
  end if;
...
comment on function public.delete_match(uuid) is
  'Suppression complète d''un match par le staff uniquement pendant les 24 heures suivant sa création. Efface le match et toutes ses données dépendantes.';
