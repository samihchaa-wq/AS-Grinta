...
-- Fusionne « Ton poste » et « Polyvalence » en un seul écran : le bilan
-- retient désormais la part de titularisations à chaque poste, et plus
-- seulement le poste le plus joué et leur nombre.

alter table public.season_wrapped
  add column if not exists position_shares jsonb not null default '[]'::jsonb;
...
comment on column public.season_wrapped.position_shares is
  ...
  '[{"position": "Milieu", "share": 62}, ...]. Les parts sont arrondies a '
  'l unite et peuvent donc ne pas totaliser exactement cent.';
