...
-- L'effectif actif ne doit filtrer que le classement de la saison courante.
-- Une adhésion de saison désactivée reste une source historique valide pour
-- Saison précédente et Toutes saisons dès lors que des feuilles de match la
-- référencent. Cette définition reprend le contrat courant de
-- v_statistics_players (vrai nom + team_clean_sheets) et déplace uniquement
-- le filtre is_active vers current_ranked.
...
comment on view public.v_statistics_players is
  'Statistiques joueurs Actuelle/Précédente/Toutes : la saison courante masque les effectifs inactifs, les périodes historiques conservent toute adhésion ayant participé à un match ; les joueurs sont nommés par leur vrai nom et team_clean_sheets compte les matchs joués avec score_adverse = 0.';
