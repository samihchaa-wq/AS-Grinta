-- The global notification kill switch must also reject explicit admin sends.
-- Otherwise the RPC returns a recipient count and the Flutter UI reports a
-- successful send while the Edge Function silently drops the request.
