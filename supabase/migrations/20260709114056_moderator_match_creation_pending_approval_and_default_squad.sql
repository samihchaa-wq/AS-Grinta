...
alter table public.profiles add constraint profiles_status_check check (status in ('pending','active','archived'));
...
begin
  insert into public.profiles(
    ...
    'pronostiqueur',
    false,
    case when coalesce((new.raw_user_meta_data->>'approval_required')::boolean, false)
...
with check ((public.is_admin() or public.is_moderator()) and created_by = auth.uid());
