-- Moteur de cotes V6 : reprend le moteur V5 (résultats pondérés par
-- récence + confrontations directes) et réintroduit le lieu du match
-- (domicile/extérieur), qui avait été retiré par erreur en V5.
--
-- La forme générale de l'AS Grinta est ajustée par sa forme spécifique au
-- lieu du match à venir (même pondération de récence, demi-vie de 180
-- jours), amortie par la quantité de données disponibles à ce lieu et
-- bornée à ±0.15 en probabilité pour éviter qu'un petit échantillon ne
-- fasse dérailler les cotes.
...
declare
  ...
  v_result jsonb;
begin
  select id, opponent_id, match_date, location, status
  into v_match
  from public.matches
  where id = p_match_id;
  ...
  v_result := public.calculate_match_odds_v6(
    v_match.opponent_id, v_match.match_date, v_match.location
  );
