...
-- Cette version existe déjà en production : elle est reconstituée ici pour
-- réaligner l'historique GitHub et garantir qu'un rejeu depuis zéro possède le
-- même correctif. Depuis l'ajout des faits durables du compte rendu,
-- delete_match doit les retirer avant les participants auxquels ils référencent.
...
comment on function public.delete_match(uuid) is
  'Suppression complète d''un match par le staff jusqu''à 24 heures après son coup d''envoi. Efface le match et toutes ses données dépendantes.';
