-- Keep every administrative match-edit path aligned with the Flutter T-15 lock.
-- Once the Live window opens, direct RPC calls must not be able to mutate
-- match identity, effectif, convocations, guests, or internal composition.
...
begin
  ...
  select m.status, m.kickoff_at, m.updated_at
...
begin
  ...
  select match_type, status, kickoff_at
...
-- Remove a staging-only legacy jersey overload; current clients use _v2.
...
begin
  ...
  perform private.assert_match_admin_edit_open(p_match_id);
  return private.recompute_match_convocations_internal(
...
begin
  ...
  perform private.assert_match_admin_edit_open(p_match_id);
  return private.configure_match_sport_workflow(
...
revoke all on function public.update_internal_match(
  uuid, uuid, date, time without time zone, text, timestamp with time zone
) from public, anon;
...
grant execute on function public.cancel_match(uuid)
to authenticated, service_role;
...
revoke all on function public.admin_publish_match_convocations(
  uuid, text
) from public, anon;
grant execute on function public.admin_publish_match_convocations(
  uuid, text
) to authenticated, service_role;

revoke all on function public.admin_recompute_match_convocations(
  uuid, boolean
) from public, anon;
grant execute on function public.admin_recompute_match_convocations(
  uuid, boolean
) to authenticated, service_role;
...
revoke all on function public.admin_configure_match_sport_workflow(
  uuid, integer
) from public, anon;
grant execute on function public.admin_configure_match_sport_workflow(
  uuid, integer
) to authenticated, service_role;
