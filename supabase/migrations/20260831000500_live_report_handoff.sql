...
-- `private.get_match_sport_report` reste la source des règles d'effectif et de
-- correction. La façade garde explicitement la même autorisation avant de lire
-- les données Live avec des privilèges élevés.
...
begin
  ...
  if not private.is_admin()
     and not private.is_match_coach_or_admin(p_match_id) then
