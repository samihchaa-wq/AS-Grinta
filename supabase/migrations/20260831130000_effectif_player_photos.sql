-- Expose la photo de profil dans le payload d'effectif administrateur afin
-- d'afficher l'avatar réel à gauche du prénom. Les invités réutilisent leur
-- photo de catalogue quand elle existe.
  ...
  left join lateral (
    ...
    from (
      select
        ...
        coalesce(profile.photo_url, guest.photo_url) as photo_url,
