-- Autorise a nouveau les echanges titulaire/remplacant avant le coup d'envoi.
--
-- Probleme constate en production : sur l'ecran de preparation du Tableau
-- Blanc (session Live a l'etat `not_started`), glisser un remplacant sur le
-- terrain echouait systematiquement avec « A field/bench change was not
-- declared as a substitution », et l'application affichait « Action non
-- confirmee. L'etat reel du serveur a ete recharge. ». La composition du coup
-- d'envoi n'etait donc plus corrigeable une fois l'espace Live ouvert.
--
-- Cause : private.save_match_live_lineup refuse tout franchissement de la
-- frontiere terrain/banc qui n'est pas declare comme remplacement. Ce garde-fou
-- protege les statistiques (Faits du match, compteur de fois sur le banc) mais
-- n'a aucun sens tant que le match n'a pas demarre : aucun evenement Live ne
-- peut exister avant le coup d'envoi.
--
-- Correctif : le controle ne s'applique plus qu'a partir du moment ou le match
-- est lance (running / paused / halftime). Le reste de la fonction, y compris
-- la validation des remplacements declares, est inchange.
...
create or replace function private.save_match_live_lineup(
  p_match_id uuid,
  p_entries jsonb,
  p_substitution jsonb default null::jsonb
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  --
  -- Avant le coup d'envoi (`not_started`) il n'y a encore ni chronometre ni
  -- remplacement possible : echanger un titulaire et un remplacant est une
  -- simple correction de composition. Exiger une declaration de remplacement
  -- rendait alors l'ecran de preparation inutilisable, et aucune statistique
  -- n'est en jeu puisque aucun evenement ne peut exister a ce stade.
...
comment on function private.save_match_live_lineup(uuid, jsonb, jsonb) is
  'Sauvegarde la composition Live. Avant le coup d''envoi les echanges terrain/banc sont de simples corrections ; une fois le match lance ils doivent etre declares comme remplacements.';
