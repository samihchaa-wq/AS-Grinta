-- Season prediction titles must use the same scoring formula as the visible
-- leaderboards. In particular:
-- - Buteurs includes the ordering bonus;
-- - Global weights Match points x100 before adding Buteurs points.
-- Without this, a player could lead the visible Global table and still lose
-- best_pred_overall to someone ranked below them.
...
begin
  ...
  pmatch_cnt as (
    select mp.profile_id,
           count(*) filter (where mp.is_filled) as cnt
  ...
  poverall as (
    select coalesce(pm.profile_id, pp.profile_id) as profile_id,
           coalesce(pm.pts, 0::numeric) * 100::numeric
             + coalesce(pp.pts, 0::numeric) as total
    from pmatch_pts pm
  ...
  w_present as (
    select profile_id, 'most_present' from (
      select profile_id, rank() over (order by n desc) rk
      from played
      where n > 0
    ) z where rk = 1
  ),
  w_scorer as (
    select profile_id, 'top_scorer' from (
      select profile_id, rank() over (order by g desc) rk
      from goals
      where g > 0
    ) z where rk = 1
  ),
  w_mvp as (
    select profile_id, 'mvp_king' from (
      select profile_id, rank() over (order by c desc) rk
      from mvp
      where c > 0
    ) z where rk = 1
  ),
  w_winrate as (
    select profile_id, 'best_winrate' from (
      select profile_id,
             rank() over (order by (w::numeric / n) desc) rk
      from played
      where n >= 5
    ) z where rk = 1
  ),
  ...
  w_pred_player as (
    select profile_id, 'best_pred_player' from (
      select profile_id, rank() over (order by pts desc) rk
      from pplayer_pts
      where pts > 0
    ) z where rk = 1
  ),
  w_pred_overall as (
    select profile_id, 'best_pred_overall' from (
      select profile_id, rank() over (order by total desc) rk
      from poverall
      where total > 0
    ) z where rk = 1
  )
