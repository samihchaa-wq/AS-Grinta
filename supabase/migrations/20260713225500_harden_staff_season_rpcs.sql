...
returns table(id uuid, first_name text, last_name text, surnom text, username text, password_set boolean, photo_url text, role text, is_goalkeeper boolean, status text, created_at timestamptz, updated_at timestamptz)
...
create or replace function public.moderator_update_profile_admin_fields(p_profile_id uuid, p_role text, p_status text, p_is_goalkeeper boolean)
returns boolean language plpgsql security definer set search_path = ''
as $$
declare
...
begin
  ...
  if current_row.role::text = 'admin' and current_row.status::text = 'active'
    ...
    if active_admins <= 1 then
      raise exception 'The last active administrator cannot be removed or archived' using errcode = '23514';
    end if;
  end if;
  update public.profiles
...
revoke execute on function public.moderator_update_profile_admin_fields(uuid, text, text, boolean) from public, anon;
...
grant execute on function public.moderator_update_profile_admin_fields(uuid, text, text, boolean) to authenticated, service_role;
...
grant execute on function public.set_season_predictions_lock(uuid, boolean) to authenticated, service_role;
