...
select ok(not has_table_privilege('authenticated','public.match_predictions','UPDATE'),
  'authenticated cannot UPDATE match_predictions directly');

-- Match prediction + reschedule contract.
...
select lives_ok(format('select public.save_match_prediction(%L::uuid,1,0)',current_setting('test.match')),
  'official match prediction RPC still writes');
...
select throws_ok(format(
  ...
  '22023','Les matchs entre nous ne sont pas ouverts aux pronostics.',
  'trigger rejects privileged internal-match writes');
...
select public.admin_update_match_complete(
  ...
  'domicile','a_venir',2,3,4,current_setting('test.updated')::timestamptz,
...
select is((select predictions_closed_at from public.matches where id=current_setting('test.match')::uuid),
  null::timestamptz,'reschedule clears stale manual prediction close');

-- Finish this fixture season so the next scenario can become the only open one.
select set_config('request.jwt.claims','{"sub":"fc100000-0000-0000-0000-000000000001","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
select public.set_season_status('fc200000-0000-0000-0000-000000000001','archived');
reset role;

-- A filled season prediction makes reveal and archive historically committed.
insert into public.seasons(id,name,status)
...
values('fc400000-0000-0000-0000-000000000001','fc200000-0000-0000-0000-000000000002',
  'Target','Season',false,true,1);
select set_config('request.jwt.claims','{"sub":"fc100000-0000-0000-0000-000000000002","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
select public.save_my_season_predictions('fc200000-0000-0000-0000-000000000002',
  jsonb_build_array(jsonb_build_object('season_player_id','fc400000-0000-0000-0000-000000000001',
...
select throws_ok(
  ...
  'filled revealed season predictions cannot be unlocked');
...
-- Archived real predictor remains in the historical leaderboard.
...
select ok(exists(select 1 from public.v_classement_general
  ...
  'archived real predictor remains in leaderboard after participation');

-- Test account is excluded before title rank, so the next real predictor wins.
