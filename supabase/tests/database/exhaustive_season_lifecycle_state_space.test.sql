...
create temporary table pg_temp.season_name_state_space(
  ...
  observed_open_count integer,
  observed_lock_cleared boolean,
...
begin
  ...
  loop
    ...
    update public.seasons
    set status = 'open',
        season_predictions_locked_at = now()
    where id = 'b6200000-0000-0000-0000-000000000001';

    begin
      v_season_id := public.open_or_create_season(v_name);
    ...
    if v_season_id is not null then
      select season.name, season.status,
        season.season_predictions_locked_at is null
      into v_observed_name, v_observed_status, v_lock_cleared
      from public.seasons season
      where season.id = v_season_id;
    else
      v_observed_name := null;
      v_observed_status := null;
    ...
    v_mismatch :=
      ...
      or (
        v_expected
        and (
          v_observed_name is distinct from btrim(v_name)
          or v_observed_status is distinct from 'open'
    ...
    if v_season_id is not null
       and v_season_id <> 'b6200000-0000-0000-0000-000000000001'::uuid then
...
select diag(format(
  'STATE_SPACE season_name case=%s name=%s expected=%s observed=%s returned_name=%s status=%s open_count=%s lock_cleared=%s sqlstate=%s message=%s',
  case_number,
  coalesce(quote_nullable(proposed_name), 'NULL'),
  expected_success,
  observed_success,
  coalesce(observed_name, '-'),
  coalesce(observed_status, '-'),
  observed_open_count,
  observed_lock_cleared,
  coalesce(observed_sqlstate, '-'),
  coalesce(observed_message, '-')
))
from pg_temp.season_name_state_space
order by case_number;

select is(
  (select count(*) from pg_temp.season_name_state_space),
  13::bigint,
  '13 formats et frontières de nom sont exécutés'
);
select is(
  (select count(*) from pg_temp.season_name_state_space where expected_success),
  5::bigint,
  'cinq noms de saison sont valides'
);
select is(
  (select count(*) from pg_temp.season_name_state_space where mismatch),
  0::bigint,
  'la création ou réouverture respecte le format, les bornes et l’unicité'
);
...
create temporary table pg_temp.season_status_state_space(
  ...
  expected_success boolean not null,
  observed_success boolean not null,
  expected_target_status text not null,
  observed_target_status text not null,
  expected_other_status text not null,
  observed_other_status text not null,
...
declare
  ...
  v_expected_open_count integer;
  v_observed_target text;
  v_observed_other text;
  v_observed_open_count integer;
  v_mismatch boolean;
begin
  foreach v_current in array array['open', 'terminee', 'archived'] loop
    foreach v_target in array array['open', 'terminee', 'archived', 'invalid', null] loop
      ...
      v_expected := v_target in ('open', 'terminee', 'archived');
      v_ok := true;
      v_sqlstate := null;
      v_message := null;
      ...
      v_expected_open_count :=
        (case when v_expected_target = 'open' then 1 else 0 end)
        + (case when v_expected_other = 'open' then 1 else 0 end);

      select status into v_observed_target
      from public.seasons
      where id = 'b6200000-0000-0000-0000-000000000001';
      select status into v_observed_other
      from public.seasons
      where id = 'b6200000-0000-0000-0000-000000000002';
      select count(*)::integer into v_observed_open_count
      from public.seasons
      where status = 'open';
      ...
      v_mismatch :=
        ...
        or v_observed_open_count is distinct from v_expected_open_count;
...
select diag(format(
  'STATE_SPACE season_status current=%s proposed=%s expected=%s observed=%s expected_target=%s observed_target=%s expected_other=%s observed_other=%s expected_open=%s observed_open=%s sqlstate=%s message=%s',
  current_status,
  coalesce(proposed_status, 'NULL'),
  expected_success,
  observed_success,
  expected_target_status,
  observed_target_status,
  expected_other_status,
  observed_other_status,
  expected_open_count,
  observed_open_count,
  coalesce(observed_sqlstate, '-'),
  coalesce(observed_message, '-')
...
select is(
  (select count(*) from pg_temp.season_status_state_space),
  15::bigint,
  '15 transitions administratives de statut sont exécutées'
);
select is(
  (select count(*) from pg_temp.season_status_state_space where mismatch),
  0::bigint,
  'toutes les transitions conservent au plus une saison ouverte'
);
...
begin
  foreach v_status in array array['open', 'terminee', 'archived'] loop
    foreach v_mode in array array['unlock', 'lock', 'null'] loop
      ...
      v_expected_locked := v_expected and v_value;
      v_ok := true;
      v_sqlstate := null;
      v_message := null;
...
select is(
  (select count(*) from pg_temp.season_lock_state_space),
  9::bigint,
  'neuf combinaisons de statut et verrou sont exécutées'
);
select is(
  (select count(*) from pg_temp.season_lock_state_space where mismatch),
  0::bigint,
...
create or replace function pg_temp.prepare_prediction_write_case(
  p_status text,
  p_locked boolean
)
returns void
...
$function$;
grant execute on function pg_temp.prepare_prediction_write_case(text, boolean)
to authenticated;
  ...
  foreach v_status in array array['open', 'terminee', 'archived'] loop
    foreach v_locked in array array[false, true] loop
      foreach v_value in array array[-1, 0, 99, 100] loop
        ...
        begin
          update public.season_predictions
          set predicted_value_30 = v_value,
              is_filled = true
          where season_id = 'b6200000-0000-0000-0000-000000000001'
            and predictor_profile_id = 'b6100000-0000-0000-0000-000000000002'
            and season_player_id = 'b6300000-0000-0000-0000-000000000001';
        ...
        v_mismatch :=
          v_ok is distinct from v_expected
          or v_observed_value is distinct from v_expected_value;
...
select diag(format(
  'STATE_SPACE season_prediction_write status=%s locked=%s value=%s expected=%s observed=%s expected_value=%s observed_value=%s sqlstate=%s message=%s',
  season_status,
  locked,
  proposed_value,
  expected_success,
...
select is(
  (select count(*) from pg_temp.season_prediction_write_state_space),
  24::bigint,
  '24 écritures de pronostic saisonnier sont exécutées sous RLS'
);
select is(
  (select count(*) from pg_temp.season_prediction_write_state_space where expected_success),
  2::bigint,
  'seules les deux valeurs valides d’une saison ouverte et déverrouillée passent'
);
select is(
  (select count(*) from pg_temp.season_prediction_write_state_space where mismatch),
  0::bigint,
  'les écritures respectent statut, verrou, propriété et bornes'
);
  ...
  foreach v_kind in array array['field', 'goalkeeper'] loop
    foreach v_category in array array['buts', 'clean_sheets'] loop
      foreach v_value in array array[-1, 0, 30, 31, 99, 100] loop
        v_player_id := case v_kind
          when 'field' then 'b6300000-0000-0000-0000-000000000001'::uuid
          else 'b6300000-0000-0000-0000-000000000002'::uuid
        end;
        v_valid_category := case v_kind
          when 'field' then 'buts'
          else 'clean_sheets'
        end;

        update public.season_predictions
        set category = v_valid_category,
            predicted_value_30 = 0,
            is_filled = false
        where season_id = 'b6200000-0000-0000-0000-000000000001'
          and predictor_profile_id = 'b6100000-0000-0000-0000-000000000002'
          and season_player_id = v_player_id;
...
select is(
  (select count(*) from pg_temp.season_prediction_validator_state_space),
  24::bigint,
  '24 combinaisons joueur, catégorie et valeur sont validées'
);
select is(
  (select count(*) from pg_temp.season_prediction_validator_state_space where expected_success),
  6::bigint,
  'six combinaisons catégorie et bornes sont valides'
);
select is(
  (select count(*) from pg_temp.season_prediction_validator_state_space where mismatch),
  0::bigint,
  'les joueurs de champ et gardiens respectent leurs catégories et plafonds'
);

create or replace function pg_temp.prepare_prediction_read_case(
  p_status text,
  p_locked boolean
)
returns void
...
$function$;
grant execute on function pg_temp.prepare_prediction_read_case(text, boolean)
to authenticated;
...
begin
  foreach v_status in array array['open', 'terminee', 'archived'] loop
    foreach v_locked in array array[false, true] loop
      perform pg_temp.prepare_prediction_read_case(v_status, v_locked);
      v_expected := v_locked or v_status = 'archived';

      select exists (
        select 1
        from public.season_predictions prediction
        where prediction.season_id = 'b6200000-0000-0000-0000-000000000001'
          and prediction.predictor_profile_id = 'b6100000-0000-0000-0000-000000000003'
          and prediction.season_player_id = 'b6300000-0000-0000-0000-000000000001'
      ) into v_visible;
...
select is(
  (select count(*) from pg_temp.season_prediction_read_state_space),
  6::bigint,
  'six états de visibilité des pronostics adverses sont exécutés'
);
select is(
  (select count(*) from pg_temp.season_prediction_read_state_space where expected_visible),
  4::bigint,
  'les pronostics adverses sont visibles après verrou ou archivage'
);
select is(
  (select count(*) from pg_temp.season_prediction_read_state_space where mismatch),
  0::bigint,
  'la lecture adverse reste secrète avant verrou sauf pour une saison archivée'
);
...
select throws_ok(
  $$select public.set_season_status(
    'b6200000-0000-0000-0000-000000000001', 'open'
  )$$,
  '42501',
  'Active administrator role required',
...
select throws_ok(
  $$select public.set_season_predictions_lock(
    'b6200000-0000-0000-0000-000000000001', true
  )$$,
  '42501',
  'Active administrator role required',
