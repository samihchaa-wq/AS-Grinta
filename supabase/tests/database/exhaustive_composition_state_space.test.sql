...
select is(
  ...
  0::bigint,
  'tous les refus de composition utilisent une erreur de validation stable'
);

-- Classes structurelles invalides.
update public.match_sport_workflows
set squad_size_limit = 16
where match_id = 'b5000000-0000-0000-0000-000000000001';
...
update public.match_sport_participants
set availability_status = 'available', convocation_status = 'convoked'
where match_id = 'b5000000-0000-0000-0000-000000000001';
update public.match_sport_participants participant
...
-- Publication initiale, republication et états de match.
...
as $function$
  select jsonb_agg(
    jsonb_build_object('season_player_id', player.id, 'status', 'convoked')
...
select is(
  ...
  '1',
  'les convocations complètes sont publiées avant la composition'
);
...
select is(
  public.admin_save_match_composition(
    'b5000000-0000-0000-0000-000000000001',
    '4-3-3',
    pg_temp.composition_payload(16, 11),
    false,
    'brouillon initial'
  ) #>> '{field_count}',
  '11',
  'le brouillon valide de seize joueurs est enregistré'
);
...
select is(
  public.admin_save_match_composition(
    'b5000000-0000-0000-0000-000000000001',
    '4-4-2',
    pg_temp.composition_payload(14, 10),
    false,
    'modification'
