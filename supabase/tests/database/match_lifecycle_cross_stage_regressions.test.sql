...
-- Ghost must be created active because the season-prediction seed contract
-- rejects a newly inserted inactive player. Deactivate him before any match is
-- created so he never belongs to a match participant snapshot.
...
select set_config(
 ...
 (select updated_at::text from public.matches
  where id=current_setting('test.cross_match')::uuid),true
);
select set_config('request.jwt.claims','{"sub":"fa100000-0000-0000-0000-000000000001","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
select ok(
...
-- Build the published lineup before the lock.
...
select public.admin_set_match_convocation(
 current_setting('test.cross_match')::uuid,
 'fa400000-0000-0000-0000-000000000002','not_convoked',true,'Cross stage'
);
...
select public.admin_save_match_composition(
 current_setting('test.cross_match')::uuid,'4-4-2',
 jsonb_build_array(
   jsonb_build_object(
...
-- Alpha withdraws after publication; Beta is promoted from waitlist.
select set_config('request.jwt.claims','{"sub":"fa100000-0000-0000-0000-000000000002","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
...
select is(
 (select convocation_status::text from public.match_sport_participants
  where match_id=current_setting('test.cross_match')::uuid
    and season_player_id='fa400000-0000-0000-0000-000000000001'),
 'not_applicable','withdrawn player leaves current convocation'
);
select is(
 (select convocation_status::text from public.match_sport_participants
  where match_id=current_setting('test.cross_match')::uuid
    and season_player_id='fa400000-0000-0000-0000-000000000002'),
 'convoked','waitlisted player is promoted'
);

-- Test-only time travel after publication to enter the T-15 Live window.
...
select is(
 ...
 'not_selected','withdrawn player does not reappear in Live'
);
select is(
 (select entry.zone::text
  from public.match_composition_entries entry
 ...
 'bench','promoted player is injected into Live bench'
);
...
-- Internal matches never enter the prediction contract.
select set_config(
 'test.internal_match',
 public.create_internal_match(
   'fa200000-0000-0000-0000-000000000001',
   ((now()+interval '1 day') at time zone 'Europe/Paris')::date,
   ((now()+interval '1 day') at time zone 'Europe/Paris')::time,null
 )::text,true
);
reset role;
select is(
 ...
 0::bigint,'internal match has no prediction rows'
);
select set_config('request.jwt.claims','{"sub":"fa100000-0000-0000-0000-000000000002","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
select throws_ok(
...
-- Create a second normal match while Alpha is active, then deactivate him.
select set_config('request.jwt.claims','{"sub":"fa100000-0000-0000-0000-000000000001","role":"authenticated","aud":"authenticated"}',true);
set local role authenticated;
select set_config(
 'test.final_match',
...
set kickoff_at=timestamp with time zone '2015-03-17 20:00:00+00',
    match_date=date '2015-03-17',match_time=time '21:00:00'
...
set local role authenticated;
select throws_ok(
 format('select public.staff_set_match_attendance(%L::uuid,array[%L::uuid])',
   current_setting('test.final_match'),'fa400000-0000-0000-0000-000000000003'),
 '22023','Player is not active in the match season',
 ...
 format(
   $sql$select public.admin_finalize_match_sport_postgame(
     ...
     (
       ...
       where participant.match_id=%L::uuid and participant.is_eligible
     ),
     'Historical inactive participant'
   )$sql$,
   current_setting('test.final_match'),current_setting('test.final_match')
 ),
...
select is(
 (select count(*) from public.match_attendance
  where match_id=current_setting('test.final_match')::uuid
    and season_player_id='fa400000-0000-0000-0000-000000000001'),
 1::bigint,'historical attendance is recorded'
);
select is(
 (select goals from public.match_player_stats
  where match_id=current_setting('test.final_match')::uuid
    and season_player_id='fa400000-0000-0000-0000-000000000001'),
 1,'historical goal survives roster deactivation'
);
