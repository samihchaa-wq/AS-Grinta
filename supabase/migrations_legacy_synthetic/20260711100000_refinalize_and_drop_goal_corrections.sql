-- La correction d'un match validé passe désormais par la même feuille de
-- statistiques que la validation initiale : finalize_match_postgame accepte
-- les matchs 'termine' et réécrit la feuille de manière atomique. L'ancienne
-- correction par événements (table goals + RPC moderator_*) était devenue
-- incohérente : le score y était recalculé depuis une table vide alors que
-- les statistiques vivent dans match_player_stats.
...
begin
  ...
  loop
    ...
    -- Appartenance au groupe de la saison : le statut du profil n'entre plus
    -- en compte pour permettre la correction d'un match d'un joueur archivé
    -- depuis.
    if not exists (
      select 1
...
-- Suppression de l'ancien flux de correction par événements.
drop function if exists public.moderator_add_match_goal(uuid, text, integer, text, uuid, text, uuid);
drop function if exists public.moderator_update_match_goal(uuid, text, integer, text, uuid, text, uuid);
drop function if exists public.moderator_delete_match_goal(uuid);
drop function if exists public.moderator_set_match_motm(uuid, uuid);
drop function if exists public.refresh_match_score_from_goals(uuid);
drop table if exists public.goals cascade;
drop table if exists public.match_correction_audit cascade;
