-- Les badges affichés à côté d'un joueur dans les Statistiques étaient reliés
-- par PRÉNOM, ce qui cassait dès qu'un joueur mettait un surnom (le nom affiché
-- devenait le surnom, plus le prénom). On expose désormais directement le
-- profile_id du compte lié dans la vue, pour un rattachement fiable côté app.
...
previous_from_import as (
  select 'previous'::text as period_key,
    ...
    coalesce(h.clean_sheets, 0) as clean_sheets, h.profile_id
...
all_time_ranked as (
  select 'all_time'::text as period_key,
    ...
    player_name, is_goalkeeper, matches_played, wins, draws, losses, goals, hdm, clean_sheets, profile_id
  from all_time_combined
)
select period_key, period_label, display_rank, display_order, player_name, is_goalkeeper,
       matches_played, wins, draws, losses, goals, hdm, clean_sheets, profile_id from current_ranked
union all
select period_key, period_label, display_rank, display_order, player_name, is_goalkeeper,
       matches_played, wins, draws, losses, goals, hdm, clean_sheets, profile_id from previous_from_app
union all
select period_key, period_label, display_rank, display_order, player_name, is_goalkeeper,
       matches_played, wins, draws, losses, goals, hdm, clean_sheets, profile_id from previous_from_import
union all
select period_key, period_label, display_rank, display_order, player_name, is_goalkeeper,
       matches_played, wins, draws, losses, goals, hdm, clean_sheets, profile_id from all_time_ranked;
