...
void main() {
  test('shared data sync reconnects and refreshes the main cached modules',
      () async {
    final sync = await File('lib/core/sync/shared_data_sync.dart').readAsString();
  ...
  test('shared data migration is a safe revision-only realtime channel',
      () async {
    final migration = await File(
      'supabase/migrations/20260802210000_shared_data_change_signal.sql',
    ).readAsString();

    expect(migration, contains('create table if not exists public.shared_data_change_signals'));
    expect(migration, contains('alter table public.shared_data_change_signals enable row level security'));
    expect(migration, contains('grant select on table public.shared_data_change_signals'));
    expect(migration, contains('private.is_active_profile()'));
    expect(migration, contains('for each statement execute function private.signal_shared_data_change()'));
