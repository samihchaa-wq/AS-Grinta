...
Duration _offsetAt(DateTime utc) {
  final start = DateTime.utc(
    utc.year,
    DateTime.march,
    _lastSunday(utc.year, DateTime.march),
    1,
  );
  final end = DateTime.utc(
    utc.year,
    DateTime.october,
    _lastSunday(utc.year, DateTime.october),
    1,
  );
...
void main() {
  group('espace d’états temporel exhaustif', () {
    test('l’ouverture J-6 tombe toujours exactement à midi Paris, chaque jour de 2025 à 2028', () {
            ...
            for (final hour in kickoffHoursUtc) {
              for (final minute in const [0, 30, 45]) {
                ...
                expect(
                  ...
                  12,
                  reason:
                      'coup d’envoi $kickoffUtc → ouverture $openAt '
                ...
                expect(
                  ...
                  expectedDate,
                  reason:
                      'coup d’envoi $kickoffUtc (Paris $kickoffLocal) : '
    ...
    test('la phase d’affichage ne recule jamais quand le temps avance', () {
          ...
          for (final liveState in liveStates) {
            ...
            for (var offset = -8 * 24 * 60; offset <= 5 * 60; offset += 10) {
              ...
              expect(
                ...
                isTrue,
                reason:
                    'retour en arrière : $kickoff (durée $duration, '
    ...
    test('les gardes d’écran restent cohérents avec la phase affichée', () {
      final kickoffs = <DateTime>[
